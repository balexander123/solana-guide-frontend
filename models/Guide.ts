...
export class Guide {
  ...
  async publicKey(): Promise<PublicKey> {
    ...
    return (
      await PublicKey.findProgramAddress(
        ...
        new PublicKey(GUIDE_REVIEW_PROGRAM_ID)
      )
    )[0]
  }
  ...
  static deserialize(buffer?: Buffer): Guide | null {
    if (!buffer) {
      return null
    }

    try {
      const { name, rating, description, reviewer } =
        this.borshAccountSchema.decode(buffer)
      return new Guide(name, rating, description, reviewer)
    } catch (e) {
